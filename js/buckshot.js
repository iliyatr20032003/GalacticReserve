...
function initBuckshotScene(){
    ...
    scene.fog = new THREE.Fog(0x0a0808, 6, 20);
    ...
    camera.position.set(0.6, 3.4, 8.6);

    const ambient = new THREE.AmbientLight(0x3d2622, 0.55);
    scene.add(ambient);

    const keyLight = new THREE.DirectionalLight(0xffcaa1, 1.15);
    keyLight.position.set(1, 7, 2);
    scene.add(keyLight);

    const rimLight = new THREE.PointLight(0x9e4b4b, 0.65, 20);
    ...
    const wallTexture = createTileTexture('#5b4a44', '#2d1f1f');
    const floorTexture = createTileTexture('#3b332e', '#161111', 18);
    const ceilingTexture = createTileTexture('#5a4b44', '#201515', 16);
    ...
    door.position.set(3.4, 1.9, -4.78);
    roomGroup.add(door);
    ...
    graffitiCtx.fillStyle = 'rgba(0,0,0,0)';
    graffitiCtx.fillRect(0, 0, graffitiCanvas.width, graffitiCanvas.height);
    graffitiCtx.font = 'bold 48px sans-serif';
    graffitiCtx.fillStyle = 'rgba(40, 20, 20, 0.7)';
    graffitiCtx.fillText('AFRAID', 18, 78);
    const graffitiTexture = new THREE.CanvasTexture(graffitiCanvas);
    const graffiti = new THREE.Mesh(
        new THREE.PlaneGeometry(2.8, 1.4),
        new THREE.MeshBasicMaterial({ map: graffitiTexture, transparent: true })
    );
    graffiti.position.set(-3.2, 2.6, -4.6);
    roomGroup.add(graffiti);
    ...
    const tableTop = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({
            map: createGrimeTexture('#55533e', '#2b261f'),
            roughness: 0.75,
            metalness: 0.05
        })
    );
    tableTop.position.set(0, 1.27, 0.2);
    scene.add(tableTop);
    ...
    feltLines.add(line1, line2);
    scene.add(feltLines);

    const ring = new THREE.Mesh(
        new THREE.RingGeometry(1.8, 1.86, 48),
        new THREE.MeshBasicMaterial({ color: 0xf2e6d1, side: THREE.DoubleSide })
    );
    ring.rotation.x = -Math.PI / 2;
    ring.position.set(0.2, 1.39, 0.2);
    scene.add(ring);
    ...
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.09, 4.2, 24), barrelMat);
    barrel.rotation.z = Math.PI / 2;
    barrel.position.set(0.3, 1.6, 0.4);
    shotgun.add(barrel);

    const stock = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.25, 0.4), new THREE.MeshStandardMaterial({
        color: 0x4b2f23,
        roughness: 0.8,
        metalness: 0.1
    }));
    stock.position.set(-1.5, 1.55, 0.4);
    shotgun.add(stock);

    const grip = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.4, 0.35), new THREE.MeshStandardMaterial({
        color: 0x3a261d,
        roughness: 0.85,
        metalness: 0.1
    }));
    grip.position.set(-0.6, 1.4, 0.4);
    shotgun.add(grip);

    shotgun.rotation.y = -0.35;
    shotgun.position.set(0.2, 0.2, -0.5);
    scene.add(shotgun);
    ...
    for(let i=0;i<3;i++){
        ...
        shell.position.set(-0.5 + i * 0.6, 1.48, -0.9);
        shellsGroup.add(shell);
    }
    scene.add(shellsGroup);

    const sinkMat = new THREE.MeshStandardMaterial({
        map: createGrimeTexture('#53463f', '#1e1412'),
        roughness: 0.9,
        metalness: 0.05
    });
    const counter = new THREE.Mesh(new THREE.BoxGeometry(3.6, 0.6, 1.2), sinkMat);
    counter.position.set(-3.3, 1.1, 1.2);
    scene.add(counter);

    const basinMat = new THREE.MeshStandardMaterial({ color: 0x2f2623, roughness: 0.7 });
    const basin = new THREE.Mesh(new THREE.CylinderGeometry(0.5, 0.5, 0.2, 24, 1, false), basinMat);
    basin.position.set(-3.3, 1.45, 1.2);
    scene.add(basin);
    ...
    const dealerBody = new THREE.Mesh(new THREE.BoxGeometry(1.4, 2.2, 0.6), dealerMat);
    dealerBody.position.set(0.2, 2.5, -3.6);
    scene.add(dealerBody);

    const dealerHead = new THREE.Mesh(new THREE.SphereGeometry(0.5, 16, 16), dealerMat);
    dealerHead.position.set(0.2, 3.7, -3.6);
    scene.add(dealerHead);

    const overhead = new THREE.Mesh(
        new THREE.BoxGeometry(2.6, 0.1, 1.2),
        new THREE.MeshStandardMaterial({ color: 0x7a6a5a, roughness: 0.6 })
    );
    overhead.position.set(0.2, 5.1, 0.6);
    scene.add(overhead);
    ...
    function animate(){
        ...
        keyLight.intensity = 1.05 * flicker;
        rimLight.intensity = 0.55 + Math.sin(elapsed * 3.5 + 1.2) * 0.08;
        camera.position.x += (targetX - camera.position.x) * 0.05;
        camera.position.y += (3.4 + targetY - camera.position.y) * 0.05;
...
function createTileTexture(baseColor, grimeColor, gridSize = 12){
    const canvas = document.createElement('canvas');
    const size = 256;
    canvas.width = size;
    canvas.height = size;
    ...
    ctx.strokeStyle = 'rgba(15, 10, 10, 0.35)';
    ctx.lineWidth = 2;
