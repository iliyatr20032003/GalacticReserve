...
function initBuckshotScene(){
    ...
    const warningPlane = new THREE.Mesh(
        new THREE.PlaneGeometry(4, 1.5),
        new THREE.MeshStandardMaterial({
            map: createGraffitiTexture('AFRAID', '#d9c7b0'),
            transparent: true
        })
    );
    warningPlane.position.set(-2, 2.8, -4.7);
    warningPlane.rotation.y = 0.1;
...
function createGraffitiTexture(text, color){
    const canvas = document.createElement('canvas');
    const width = 512;
    const height = 256;
    canvas.width = width;
